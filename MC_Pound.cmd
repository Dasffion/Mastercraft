...
action var tool bellows;put #parse @%tool when and produces less heat from the stifled coals\.|is unable to consume its fuel\.|PUSH a BELLOWs
action var tool tongs;put #parse @%tool when would benefit from some soft reworking\.|could use some straightening along the horn of the anvil\.|drawn into wire on a mandrel|^The .+ now looks ready to be turned into wire using a mandrel or mold set\.|TURN it with the tongs to perform these tasks\.$|^The metal must be transfered to plate molds and drawn into wire on a mandrel sets using tongs\.|TURN it with the tongs
action var tool shovel;put #parse @%tool when dies down and appears to need some more fuel|^The fire needs more fuel before you can do that\.|^As you complete working the fire dies down and needs more fuel|PUSH FUEL with a SHOVEL
action var tool tub;put #parse @%tool when ready for a quench hardening in the slack tub\.|^The metal now appears ready for cooling in the slack tub\.|^ You can PUSH the TUB to reposition it and quench the hot metal\.|PUSH the TUB
action var tool oil;put #parse @%tool when some oil to preserve and protect it.$|POUR OIL on .* to complete the forging process\.$
action var tool analyze when ^Applying the final touches |^That tool does not seem suitable for that task\.
action var tool hammer;put #parse @%tool when push my bellows|turn .* with my tongs|push fuel with my shovel|push fuel with my tongs|pull .* with my pliers$|push tub|looks ready to be pounded with a forging hammer.|You do not see anything that would obstruct pounding of the metal with a forging hammer
action var tool rehammer;put #parse @%tool when ^The .* appears ready for pounding
action var tool pliers;put #parse @%tool when (with the|using) pliers to (stitch|pull|rivet) them together\.$|appear ready for bending using a pair of pliers\.|are now ready for (stitching|riveting) together using pliers\.$|Just pull the .* with the pliers|(bending|weaving) of .* into and around it\.|The links appear ready to be woven into and around
action var tool assemble;put #parse @%tool when ^\[Ingredients can
